...
switch ($_POST ['action']) {
    ...
    case 'showDetails':
        ...
        $defaultEndpoint = 'http://' . $_SERVER ['SERVER_NAME'] . ':' . $_SERVER ['SERVER_PORT'] . '/sys' . SYS_SYS .
            '/'. SYS_LANG .'/classic/services/wsdl2';
...
try {
    ...
    if (isset ($_POST ['form'] ['ACTION'])) {
        ...
        $defaultEndpoint = 'http://' . $_SERVER ['SERVER_NAME'] . ':' . $_SERVER ['SERVER_PORT'] . '/sys' .
            SYS_SYS . '/'. SYS_LANG .'/classic/services/wsdl2';
        ...
        switch ($action) {
            ...
            case "CaseList":
                ...
                if (is_array($result)) {
                    foreach ($result as $key => $item) {
                        ...
                        $rows [] = array ('guid' => $guid, 'name' => $name, 'status' => $status,
                            'delIndex' => $delIndex);
                    }
            ...
            case "ReassignCase":
                ...
                $params = array ('sessionId' => $sessionId, 'caseId' => $caseId, 'delIndex' => $delIndex,
                    'userIdSource' => $userIdSource, 'userIdTarget' => $userIdTarget);
            ...
            case "NewCaseImpersonate":
                ...
                $params = array ('sessionId' => $sessionId, 'processId' => $processId, 'userId' => $userId,
                    'variables' => $variables);
            ...
            case "NewCase":
                ...
                $params = array ('sessionId' => $sessionId, 'processId' => $processId, 'taskId' => $taskId,
                    'variables' => $variables);
            ...
            case "CreateUser":
                ...
                $params = array ('sessionId' => $sessionId, 'userId' => $userId, 'firstname' => $firstname,
            ...
            case "InputDocumentList":
                ...
                if (is_array($result)) {
                    foreach ($result as $key => $item) {
                        ...
                        $rows [] = array ('guid' => $guid, 'filename' => $filename, 'docId' => $docId,
                            'version' => $version, 'createDate' => $createDate, 'createBy' => $createBy,
                            'type' => $type, 'link' => $link);
                    }
                    global $_DBArray;
                    $_DBArray = (isset ($_SESSION ['_DBArray']) ? $_SESSION ['_DBArray'] : '');
            ...
            case "wsSendFiles":
                if (isset($_FILES['form'])) {
                    foreach ($_FILES['form']['name'] as $sFieldName => $vValue) {
                        if ($_FILES['form']['error'][$sFieldName] == 0) {
                            file_put_contents(G::sys_get_temp_dir().PATH_SEP.$_FILES['form']['name'][$sFieldName],
                            file_get_contents($_FILES['form']['tmp_name'][$sFieldName]));
                ...
                function sendFile(
                    ...
                    $defaultEndpoint = 'http://' . $_SERVER ['SERVER_NAME'] . ':' . $_SERVER ['SERVER_PORT'] .
                        '/sys' . SYS_SYS . '/'. SYS_LANG .'/classic/services/upload';
