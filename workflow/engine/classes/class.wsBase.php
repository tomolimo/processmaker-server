...
class wsBase
{
    ...
    public function __construct($params=null)
    {
        ...
        if ($params != null) {
            $this->stored_system_variables = (
                isset($params->stored_system_variables)? $params->stored_system_variables : false
            );
            ...
            if ($iOcurrences) {
                for ($i = 0; $i < $iOcurrences; $i++) {
                    ...
                    $templateContents = str_replace(
                        '@>' . $sGridName . $sStringToRepeat . '@<' . $sGridName,
                        $sAux,
                        $templateContents
                    );
                }
    ...
    public function derivateCase($userId, $caseId, $delIndex, $bExecuteTriggersBeforeAssignment=false)
    {
        try {
            ...
            $oCriteria->add(
                $oCriteria->getNewCriterion(AppDelayPeer::APP_TYPE, 'PAUSE')->addOr(
                    $oCriteria->getNewCriterion(AppDelayPeer::APP_TYPE, 'CANCEL')
                )
            );
            ...
            foreach ($derive as $key => $val) {
                ...
                $varResponse = $varResponse . (
                    ($varResponse != '')? ',' : ''
                ) . $val['NEXT_TASK']['TAS_TITLE'] . $usrasgdUserName;
            }
    ...
    /**
     * execute Trigger, executes a ProcessMaker trigger. Note that triggers which are tied to case derivation
     ...
     */
    public function importProcessFromLibrary(
        $processId,
        $version='',
        $importOption='',
        $usernameLibrary='',
